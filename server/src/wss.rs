mod wss {
  ...
  fn ws_server(addr: &str, handle_conn: fn(&mut tungstenite::WebSocket<std::net::TcpStream>)) {
  ...
  fn wss_server(addr: &str, filepath: &str, handle_conn: fn(&mut tungstenite::WebSocket<native_tls::TlsStream<std::net::TcpStream>>)) {
